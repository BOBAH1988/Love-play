// games/ideas.js — Игра "Идеи для вас" (пары).
...
// но без уровней и без картинок — просто текстовые карточки-сценарии из
// cards/cards_ideas.js (IDEAS_CARDS), с избранным по аналогии с остальными играми.

let ideasCurrentCard = null;

function getIdeasPool(){
  const all = (typeof IDEAS_CARDS !== 'undefined' && Array.isArray(IDEAS_CARDS)) ? IDEAS_CARDS : [];
...
function updateIdeasFavViewBtn(){
  ...
  btn.textContent = state.ideasFavView ? '⭐ Все идеи' : '⭐ Избранное';
}
...
function drawIdeaCard(){
  ...
  if(pool.length === 0){
    ...
    fadeSwapEl('ideasCard', (el)=>{
      el.innerHTML = `<div class="card-inner"><div class="card-body"><div class="card-icon">💭</div><div class="card-text">${state.ideasFavView ? 'Пока нет избранных идей — отметьте понравившиеся ☆' : 'Идеи скоро появятся — добавьте их в cards_ideas.js'}</div></div></div>`;
    });
    updateIdeasFavBtn();
    return;
  }
  ...
  if(candidates.length === 0){
    candidates = pool;
    used = [];
    showToast('Идеи показаны заново 🔀');
  }
...
document.getElementById('ideasGameRulesBtn').addEventListener('click', ()=>{ document.getElementById('ideasRulesModal').classList.add('show'); });
