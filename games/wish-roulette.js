...
function spinWishWheel(){
  ...
  wishSpinTimer = setTimeout(()=>{
    ...
    // В режиме «Интересная» цвет сектора определяет исполнителя: ⚫ чёрный —
    // мужчина, 🔴 красный — женщина, 🟢 зеро — общее задание для пары.
    // В режиме «Простая» исполнителя определяет очередь ходов (см. ниже).
    ...
    if(resultEl){
      ...
      resultEl.innerHTML = `
        ...
        ${whoLine ? `<div style="font-size:18px;font-weight:700;margin:0 0 6px;color:${who==='both'?'#7cfc9b':(level===6?'#fff':'#ffd23f')};">${whoLine}</div>` : ''}
