...
function checkMarkup(html) {
  ...
  // Значок «+» у кнопки импорта видео — отдельный элемент, а не эмодзи ➕:
  // цвет эмодзи через CSS не задаётся, а «+» должен быть зелёным (v436).
  check('у «Добавить свое видео» значок «+» — отдельный элемент',
    /id="davaySetupImportBtn"[^>]*>[\s\S]*?<span class="davay-add-plus"/.test(html),
    'значок «+» снова эмодзи внутри подписи — CSS-цвет зелёным не применится');
  // У кнопки «Звук» значок тоже вынесен: он крупнее подписи (см. CSS).
  check('у кнопки «Звук» значок вынесен отдельным элементом',
    /id="davaySetupSoundBtn"[^>]*>[\s\S]*?<span class="davay-sound-icon"/.test(html),
    'значок звука склеен с подписью — CSS-размер к нему не применится');
...
function checkStyles(html) {
  ...
  // Значок «+» кнопки «Добавить свое видео» («Давай попробуем») — зелёный.
  // Цвет задан правилом по классу значка: если правило потеряется, плюс
  // станет белым, как остальная подпись.
  const addPlusRule = css.match(/#davaySetupImportBtn \.davay-add-plus\{([^}]*)\}/);
  check('правило зелёного «+» у кнопки импорта видео есть в CSS',
    !!addPlusRule, 'нет правила #davaySetupImportBtn .davay-add-plus — значок «+» останется белым');
  check('«+» у кнопки импорта видео зелёный',
    !!addPlusRule && /color:\s*#4cd964/.test(addPlusRule[1]),
    `цвет значка «+»: ${addPlusRule ? addPlusRule[1].trim() : 'правила нет'} — ожидался зелёный #4cd964`);
  check('«+» у кнопки импорта видео крупнее подписи на 50% (25.5px)',
    !!addPlusRule && /font-size:\s*25\.5px/.test(addPlusRule[1]),
    `размер «+»: ${addPlusRule ? addPlusRule[1].trim() : 'правила нет'} — ожидался 25.5px (17px + 50%)`);
  // Значок звука у кнопки «Звук» той же строки — тоже крупнее подписи (13px).
  const soundIconRule = css.match(/#davaySetupSoundBtn \.davay-sound-icon\{([^}]*)\}/);
  check('правило значка звука у кнопки «Звук» есть в CSS',
    !!soundIconRule,
    'нет правила #davaySetupSoundBtn .davay-sound-icon — значок останется размером с подпись');
  check('значок звука у кнопки «Звук» крупнее подписи на 50% (19.5px)',
    !!soundIconRule && /font-size:\s*19\.5px/.test(soundIconRule[1]),
    `размер значка звука: ${soundIconRule ? soundIconRule[1].trim() : 'правила нет'} — ожидался 19.5px (13px + 50%)`);
  ...
  // Подпись кнопки «Звук» на настройке «Давай попробуем» JS перерисовывает
  // при каждом переключении: если вернуть сюда textContent, вложенный значок
  // <span> пропадёт и он снова станет размером с подпись.
  check('значок звука на настройке «Давай попробуем» — отдельный элемент',
    /class="davay-sound-icon"/.test(sndDavay),
    'updateDavaySetupSoundBtn снова пишет textContent — значок не увеличить через CSS');
