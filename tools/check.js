...
function checkStyles(html) {
  ...
  check('«Предложи партнеру»: иконки и подсказки переставлены — 📶 «По порядку», 🔀 «Случайный порядок»',
    /ordered \? '🔀' : '📶'/.test(photoCore)
      && /btn\.dataset\.tt = ordered \? 'Случайный порядок' : 'По порядку'/.test(photoCore)
      && /MIGRATIONS\[3\] = function\(s\)\{\s*\n\s*s\.photoOrderMode = false;/.test(photoCore),
    'updatePhotoRandomToggleBtn (иконки/подсказки) или MIGRATIONS[3] не соответствуют запросу владельца');
  check('«Предложи партнеру»: разметка кнопки соответствует дефолту (📶 «По порядку»)',
    /id="photoRandomToggleBtn"[^>]*data-tt="По порядку"[^>]*>📶</.test(html)
      && /id="photoRandomToggleBtn"[^>]*aria-label="По порядку/.test(html),
    'статическая разметка кнопки приведена не к дефолтному состоянию — до первого вызова updatePhotoRandomToggleBtn игрок видит не ту подсказку');
