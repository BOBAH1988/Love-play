...
test('«Арифметика»: карточка собрана как у остальных обучающих игр', () => {
  ...
  assert(/\.znayu-answers\s+\.znayu-answer-btn\s*\{[^}]*background:rgba\(255,255,255,\.3\);\s*color:#2b0f2e/.test(css),
    'все кнопки ответов должны использовать единый полупрозрачный белый фон 30% с тёмным текстом');
});
