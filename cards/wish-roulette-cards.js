...
(function(){
  ...
  const SIMPLE_TASKS = [
    'поцелуй щеку', 'лизни живот', 'потрогай губы', 'поцелуй шею', 'лизни руку',
    'потрогай ногу', 'поцелуй грудь', 'потрогай бедро', 'лизни спину', 'поцелуй ступню',
    'потрогай подбородок', 'лизни лоб', 'поцелуй ладонь', 'потрогай запястье', 'лизни плечо',
    'поцелуй колено', 'потрогай голову', 'лизни ухо', 'поцелуй нос', 'потрогай глаза',
    'обними щеку', 'погладь живот', 'потри шею', 'обними руку', 'погладь бедро',
    'потри спину', 'обними подбородок', 'погладь плечо', 'потри колено', 'обними ступню',
    'погладь лоб', 'потри запястье', 'обними ладонь', 'погладь голову', 'потри ухо',
    'обними нос', 'погладь глаза'
  ];

  const simpleCards = [];
  for (let n = 0; n < 37; n++) {
    const who = (n === 0) ? 'both' : (RED.has(n) ? 'F' : 'M');
    simpleCards.push({ level: 0, type: 'dare', text: SIMPLE_TASKS[n], number: n, who: who });
  }
