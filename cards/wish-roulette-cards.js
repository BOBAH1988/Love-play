...
//
// Режим «Простая» (state.wrGameMode === 'simple'):
//   window.WISH_ROULETTE_SIMPLE_CARDS — 37 статичных заданий, каждое из 2 слов
//   (действие + часть тела), по одной ячейке колеса. Поле who в карточках
//   оставлено для совместимости, но в «Простой» НЕ используется: исполнитель
//   определяется очерёдностью ходов партнёров (см. games/wish-roulette.js).
...
(function(){
  ...
  // ── Режим «Простая»: пул статичных заданий для всех 37 ячеек колеса ──
  // Каждое задание — ровно 2 слова: 1 действие + 1 часть тела.
  // Цвет сектора определяет исполнителя: ⚫ чёрный — мужчина, 🔴 красный —
  // женщина, 🟢 зеро — общее задание для пары.
  const SIMPLE_TASKS = [
    'Поцелуй в губы', 'Сожми попу', 'Погладь шею', 'Лизни грудь', 'Шлёпни попу',
    'Укуси шею', 'Поцелуй лоб', 'Сожми грудь', 'Погладь живот', 'Лизни живот',
    'Шлёпни бедро', 'Укуси губу', 'Укуси язык', 'Сожми поясницу', 'Погладь ногу',
    'Лизни шею', 'Шлёпни ногу', 'Укуси грудь', 'Поцелуй живот', 'Сожми промежность',
    'Погладь попу', 'Лизни поясницу', 'Шлёпни живот', 'Укуси попу', 'Поцелуй грудь',
    'Сожми ногу', 'Погладь промежность', 'Ущипни попу', 'Шлёпни поясницу', 'Укуси живот',
    'Поцелуй шею', 'Сожми живот', 'Погладь грудь', 'Лизни ногу', 'Подуй на грудь',
    'Укуси поясницу', 'Поцелуй поясницу', 'Сожми попу', 'Погладь губы', 'Поцелуй промежность'
  ];

  const simpleCards = [];
  for (let n = 0; n < 37; n++) {
    const who = (n === 0) ? 'both' : (RED.has(n) ? 'F' : 'M');
    simpleCards.push({ level: 0, type: 'dare', text: SIMPLE_TASKS[n % SIMPLE_TASKS.length], number: n, who: who });
  }

  window.WISH_ROULETTE_CARDS = all;
  window.WISH_ROULETTE_CARDS_BY_LEVEL = byLevel;
  window.WISH_ROULETTE_SIMPLE_CARDS = simpleCards;
})();
